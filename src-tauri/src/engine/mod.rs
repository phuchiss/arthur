...
pub enum LogEvent {
    ...
    ///
    /// `request_id` is generated by Arthur. In workflow runs the UI replies
    /// via the `respond_ask` Tauri command (the runner uses the id to match
    /// answers to the right pending question); chat ignores it.
    AskUserQuestion {
        step_id: String,
        request_id: String,
        questions: Vec<UserQuestion>,
    },
    /// Emitted by an `interactive: true` ACP step after each agent turn that
    /// did *not* call `AskUserQuestion`. The UI must surface a reply composer:
    /// sending text continues the turn loop with that text as the next prompt,
    /// dismissing (`null` reply) ends the step with the accumulated text.
    AwaitingMessage {
        step_id: String,
        request_id: String,
        /// The agent's final assistant text for the turn that just ended —
        /// gives the UI something to show above the composer ("you're
        /// replying to: ...") without re-reading the streamed stdout.
        last_text: String,
    },
